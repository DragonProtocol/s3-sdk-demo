...
function RainbowKitAuth({ children }: PropsWithChildren) {
  ...
  const { us3rAuth, us3rAuthValid, connectUs3r } = useUs3rProfileContext()!;
  ...
  }, [us3rAuth, us3rAuthValid, disconnect]);
    ...
    async onConnect({ connector, isReconnected }) {
      try {
        ...
        if (!isReconnected && provider) {
          await loginWithAuthorizer(AuthToolType.rainbowKit, provider);
          // 脱离authkit，就用下面这个
